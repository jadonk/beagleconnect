...
WARNING: TODO provide images demonstrating Jupyter Notebook visualization

For embedded systems developers, data is easily extracted using the standard
...
WARNING: TODO provide image illustrating remote management#
...
WARNING: TODO think a bit more about this section with some feedback from Cathy.
...
WARNING: TODO provide image of BeagleConnect™ Freedom in case with a hand for size perspective#
...
image::graphics/BeagleConnectFreedom-Front-Annotated-wbg.png[]
image::graphics/BeagleConnectFreedom-Back-Annotated-wbg.png[]
...
unusable by my mom. She typed well, but the dexterity to move a mouse alluded her.
...
* `BeagleConnect™ node device` function for Bluetooth Low-Energy (BLE) and Sub-GHz
...
WARNING: TODO provide BeagleConnect™ Freedom pinout diagram
...
WARNING: TODO Actually, Node-RED will make these show up automatically as streams.
...
WARNING: TODO Find a place for the following notes:
...
WARNING: TODO describe how to know it is working
...
** The default IP for the BeagleBone hardware over USB is `192.168.7.2` (over Ethernet it's usually at beaglebone.local)
...
3. Install https://github.com/rcn-ee/code-server/tree/v3.11.1-bbb.io[Visual Studio Code Server] for easy terminal access and code editing.
...
** Enable and start the VSCode Server on `192.168.7.2:3000`
  
  sudo systemctl enable bb-code-server
  
  sudo systemctl start bb-code-server

** Visit `http://192.168.7.2:3000` and access the terminal easily using `CTRL + ``, You may also checkout other VSCode shortcuts https://code.visualstudio.com/shortcuts/keyboard-shortcuts-linux.pdf[here].

image::graphics/bb-code-server-terminal.png[]

WARNING: TODO the above installation of bb-code-server should not be required on the mikrobus images. it needs to be enabled by default.

NOTE: If you are facing some issue during boot then you can try debuging the boot session with a USB to serial interface cable such as those made by https://www.digikey.com/short/cfjmdbdd[FTDI] plugged into J10 with the black wire of the FTDI cable toward the Ethernet connector. Application like tio/minicom/putty can be used to make the connection establishment procedure easy.

WARNING: TODO Simplify and elaborate on this section, add boot session debugging walkthrough
...
image::graphics/bcf_block_diagram.png[Block Diagram]
...
WARNING: TODO The agenda for the above falls a bit short of BeagleConnect
...
WARNING: TODO Fill in some details for new embedded Linux users

WARNING: TODO Arrange demo in order of _fast success_ to _greater understanding_
...
5. Log onto the Seeed BeagleBone® Green Gateway using `ssh` or, with a serial to USB cable such as those made by FTDI[https://www.digikey.com/short/cfjmdbdd] plugged into J10 with the black wire of the FTDI cable toward the Ethernet connector.  To use `ssh`, you will have to know the Green Gateway's IP address that can be found from your router.
6. (If using WiFi) Use the `connmanctl` tool to gain an Internet connection.

=== Flash BeagleConnect™ Freedom *gateway* device

Command-line instructions below are to be issued into the BeagleBone® Green Gateway terminal, either via `ssh` or the serial console.
...
WARNING: TODO How can we add a step in here to show the network is connected without needing `gbridge` to be fully functional?
...
WARNING: TODO can we also handle the case where these sensors are included and recommend them? Same firmware?

WARNING: TODO the current demo is for the built-in sensors, not the Click boards mentioned below
...
WARNING: TODO this log needs to be updated
...
WARNING: TODO bring in the GPIO toggle and I2C explorations for greater understanding
...
WARNING: TODO revisit everything below here
...
WARNING: TODO The patches for gb-netlink will eventually be applied here until pushed into mainline.#
