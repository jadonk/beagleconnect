...
----
+
2. MSP430 flashing utility
+
[source,bash]
----
cd /tmp/beagleconnect/sw/ti_msp430_python_tools/python-msp430-tools
python2 setup.py install --user
----
+
3. CC1352 Zephyr toolchain
+
[source,bash]
----
...
[source,bash]
----
cd /tmp/beagleconnect/sw/usb_uart_bridge
MSP430_TOOLCHAIN_PATH=/opt/msp430-gcc make
cd /tmp/beagleconnect/sw/usb_wpan_temp
MSP430_TOOLCHAIN_PATH=/opt/msp430-gcc make
----
+
...
----
cd /tmp/beagleconnect/sw/zephyrproject/zephyr
west build -p always -b cc1352r1_launchxl samples/subsys/greybus/net -d build/greybus_launchpad -- -DCONF_FILE="prj.conf overlay-802154-subghz.conf"
...
[source,bash]
----
cd /tmp/beagleconnectd/sw/buildroot
make
----
...
+
2. Flash MSP430 (with temporary firmware for flashing CC1352) and CC1352
+
NOTE: Make sure no instances of `tio` or `qemu` are running that would capture the serial port.
+
NOTE: Be sure to replace `/tmp` and `/dev/ttyACM0` with appropriate values for your setup.
+
[source,bash]
----
cd /tmp/beagleconnect/sw/usb_uart_bridge
...
+
5. Flash MSP430 with gateway software
+
[source,bash]
----
cd /tmp/beagleconnect/sw/usb_wpan_temp
